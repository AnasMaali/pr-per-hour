...
import type { ServiceFieldErrors } from '@/features/admin/services/types/adminServices.types'
...
export function mapServiceApiError(error: unknown): MappedServiceError {
  ...
  if (normalized.errors) {
    for (const [key, messages] of Object.entries(normalized.errors)) {
      ...
      if (message) {
        fieldErrors[key as keyof ServiceFieldErrors] = message
      }
    }
  }

  if (normalized.errorCode === 'CATEGORY_UNAVAILABLE') {
    return {
      fieldErrors,
      formMessageKey: 'errorCategoryUnavailable',
