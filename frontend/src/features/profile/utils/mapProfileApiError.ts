...
const FIELD_KEYS = new Set(['name', 'phone'])
...
export function mapProfileApiError(error: unknown): MappedProfileError {
  ...
  if (normalized.errors) {
    for (const [key, messages] of Object.entries(normalized.errors)) {
      ...
      if (message) {
        fieldErrors[key as keyof ProfileFieldErrors] = message
      }
    }
  }

  if (normalized.isInactiveAccount || normalized.errorCode === 'INACTIVE_ACCOUNT') {
    return {
      fieldErrors,
